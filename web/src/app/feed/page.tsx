...
type PostRow = {
  ...
  user_id: string;
  content: string | null;
  media_url: string | null; // Keep for backward compatibility
  attachments: Attachment[] | null; // NEW: JSONB array
  created_at: string;
...
export default function FeedPage() {
  ...
  // For editing media, we might need more complex state, but MVP:
  // We will allow adding *new* files to replace/append to existing? 
  // For simplicity: Editing = Editing text only in MVP+, but user asked for video edit.
  // To allow video edit, we need to let them clear attachments and upload new ones.
  ...
  async function loadPosts() {
    try {
      ...
      let query = supabase
        .from("posts")
        .select(
          "id,user_id,content,media_url,attachments,created_at,profiles(username,full_name,avatar_url),post_likes(user_id),post_comments(id)"
        )
      ...
      toast.error(e?.message ?? "Failed to load feed");
    }
  }
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>, isEdit = false) => {
    ...
    // We need to count existing text attachments if editing? 
    // Logic: 2 videos + 2 images OR 4 images total.

    // Simplification for UX: Just limit total items to 4, max 2 videos.
    let videos = 0;
    let images = 0;

    // Count existing (for edit mode, we check editAttachments too)
    if (isEdit) {
    ...
    for (const f of files) {
      ...
      if (isVideo) {
        if (videos < 2 && (videos + images) < 4) {
          videos++;
          validFiles.push(f);
        } else {
          toast.error("Limit reached: Max 2 videos.");
        }
      } else {
        if ((videos + images) < 4) {
          images++;
          ...
          toast.error("Limit reached: Max 4 items.");
        }
      }
    }
  ...
  async function createPost() {
    ...
    try {
      ...
      if (newFiles.length > 0) {
        attachments = await uploadPostMedia(newFiles, myId);
      }

      const { error } = await supabase.from("posts").insert({
        user_id: myId,
        content: text || null,
        media_url: attachments.length > 0 ? attachments[0].url : null, // Fallback for old clients
  ...
  // ... (Like, Comment, Delete functions essentially unchanged) ...
  function isLikedByMe(p: PostRow) {
    return (p.post_likes ?? []).some((l) => l.user_id === myId);
  }
  ...
  async function openComments(postId: string) {
    ...
    const { data } = await supabase.from("post_comments")
      .select("id,post_id,user_id,content,created_at,profiles(username,full_name,avatar_url)")
      .eq("post_id", postId)
  ...
  async function addComment() {
    ...
    try {
      await supabase.from("post_comments").insert({ post_id: openCommentsFor, user_id: myId, content: commentText.trim() });
      setCommentText("");
  ...
  async function deletePost(postId: string) {
    ...
    try { await supabase.from("posts").delete().eq("id", postId); toast.success("Deleted"); }
  ...
  function startEditing(p: PostRow) {
    ...
    // Fallback: if no attachments array but media_url exists, create one
    let existing: Attachment[] = p.attachments || [];
    if (existing.length === 0 && p.media_url) {
      // guess type
      const isVid = p.media_url.includes(".mp4") || p.media_url.includes("video"); // heuristic
  ...
  async function savePostEdit(postId: string) {
    try {
      ...
      if (editNewFiles.length > 0) {
        const newUploaded = await uploadPostMedia(editNewFiles, myId!);
      ...
      const changes = {
        ...
        // Update legacy field if it was empty or changed? 
        // We'll just sync the first attachment to media_url for safety
        media_url: finalAttachments.length > 0 ? finalAttachments[0].url : null
      };
  ...
  async function blockUser(userId: string) {
    if (!myId || !confirm("Block user?")) return;
    ...
    <main className="min-h-screen">
      <div className="mx-auto max-w-2xl px-4 py-8 space-y-6">
        ...
        <Card className="p-5 rounded-xl border-border/60 shadow-sm">
          <div className="font-semibold text-lg mb-2">Create post</div>
          <div className="space-y-4">
            <Textarea
              value={newText}
              onChange={(e) => setNewText(e.target.value)}
              placeholder="What's happening?"
              className="resize-none min-h-[100px] bg-muted/30 border-none focus-visible:ring-1"
            />
        ...
        {posts.map((p) => {
          ...
          // Helper to get attachments logic
                ...
                <div className="flex justify-between items-start">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      {p.user_id === myId ? (
                        <>
                          <DropdownMenuItem onClick={() => startEditing(p)}><Edit className="mr-2 h-4 w-4" /> Edit</DropdownMenuItem>
                        ...
                        <DropdownMenuItem onClick={() => blockUser(p.user_id)} className="text-destructive"><Ban className="mr-2 h-4 w-4" /> Block</DropdownMenuItem>
                      )}
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
                ...
                <div className="mt-3">
                  {isEditing ? (
                    <div className="space-y-4">
                      ...
                      {/* Existing Attachments in Edit */}
                      {editAttachments.length > 0 && (
                        <div>
                          <div className="text-xs font-semibold mb-2">Current Media:</div>
                          <div className="grid grid-cols-2 gap-2">
                            {editAttachments.map((a, i) => (
                              <div key={i} className="relative aspect-video bg-black rounded-md overflow-hidden group">
                                ...
                                <button onClick={() => removeEditAttachment(i)} className="absolute top-1 right-1 bg-red-500 text-white rounded-full p-1"><X className="h-3 w-3" /></button>
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      {editNewFiles.length > 0 && (
                        <div>
                          <div className="text-xs font-semibold mb-2 mt-2">New Uploads:</div>
                      ...
                      <div className="flex justify-end gap-2">
                        ...
                        <Button size="sm" onClick={() => savePostEdit(p.id)}>Save Changes</Button>
                      </div>
                    </div>
                  ) : (
                    <>
                      ...
                      {displayAttachments.length > 0 && (
                        <div className={cn("mt-3 grid gap-2 overflow-hidden rounded-xl",
                          displayAttachments.length === 1 ? "grid-cols-1" : "grid-cols-2"
                        )}>
                          {displayAttachments.map((a, idx) => (
                            <div key={idx} className={cn("relative bg-black/50 overflow-hidden",
                              a.type === 'video' ? "aspect-video" : "aspect-square"
                            )}>
                              {a.type === 'video' ? (
                                <video src={a.url} controls playsInline className="w-full h-full object-contain" />
                              ) : (
                ...
                <div className="mt-4 flex items-center justify-between border-t border-border/30 pt-3">
                  <div className="flex gap-1">
                    <Button variant="ghost" size="sm" onClick={() => toggleLike(p.id)} className={cn("h-8 rounded-full gap-2", liked && "text-primary")}>
                      <ThumbsUp className={cn("h-4 w-4", liked && "fill-current")} /> <span className="text-xs">{p.post_likes?.length || "Like"}</span>
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => openComments(p.id)} className={cn("h-8 rounded-full gap-2", openCommentsFor === p.id && "text-blue-500")}>
                      <MessageSquare className="h-4 w-4" /> <span className="text-xs">{p.post_comments?.length || "Comment"}</span>
                    </Button>
                  </div>
